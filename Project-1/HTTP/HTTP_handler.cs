...
namespace multithreaded_web_server.HTTP
{
    class HttpRequest
    {
    ...
    {
        private static HttpRequest ParseHttpRequest(StreamReader reader)
        {
        ...
        public static void HandleClient(object o)
        {
            ...
            HttpRequest parsed_http_request = ParseHttpRequest(reader);
            ...
            */
            ...
            try
            {
                if (status_line_parts[0] != "GET")
                {
                    ...
                    Logger.Log($"[{clientIP}] requested {status_line_parts[0]} {status_line_parts[1]} -> 405 Method Not Allowed");
                    client.Close();
                    return;
                }
                ...
                if (cache.TryGet(filename, out string cachedResponse))
                {
                    writer.Write(cachedResponse);
                    writer.Flush();
                    Logger.Log($"[{clientIP}] GET {filename} -> 200 OK [CACHED]");
                    return;
                }

                if (FileHandler.ConvertFileBinaryTxt(filename))
                {
                    string body = "Conversion finished successfully.";
                    responseToSend = FormatHttpResponse((int)HttpStatusCode.OK, body);
                    Logger.Log($"[{clientIP}] converted {filename} successfully.");
                } 
                else
                {
                    string body = "Conversion has already been done for this file.";
                    responseToSend = FormatHttpResponse((int)HttpStatusCode.OK, body);
                    Logger.Log($"[{clientIP}] requested {filename} -> already converted.");
                }

                writer.Write(responseToSend);
                writer.Flush();

                cache.Add(filename, responseToSend);
            }
            catch (FileNotFoundException ex)
            {
                responseToSend = FormatHttpResponse((int)HttpStatusCode.NotFound);
                writer.WriteLine(responseToSend);
                Logger.Log($"[{clientIP}] requested {status_line_parts[0]} {status_line_parts[1]} -> 404 Not Found\n" +
                    $"({ex.Message})");
            ...
            {
                writer.WriteLine(FormatHttpResponse((int)HttpStatusCode.BadRequest));
                Logger.Log($"[{clientIP}] requested {filename} -> 400 Bad Request ({ex.Message})");
            }
            catch (Exception ex)
            {
                writer.WriteLine(FormatHttpResponse((int)HttpStatusCode.InternalServerError, null, null, ex.Message));
                Logger.Log($"[{clientIP}] requested {status_line_parts[1]} -> 500 Internal Server Error " +
                    $"({ex.Message})");
            }
            finally 
            {
